...
data_df = pd.read_csv('log_data6.csv', index_col=0)
...
Time_df = pd.read_csv('log_time6.csv', index_col=0)
...
def run_analysis(k, data):
    ...
    result = fun_clu(data, k, Time, n1=15)
...
# for k in list(range(159,160,1)):
for k in [159,161,165,166,167,168]:
    for i in list(range(7, 31, 1)):
        print(f"开始运行 k={k} 的分析...")
        result = fun_clu(data, k, Time, n1=15)
        result['par'] = result['par'].tolist()
        result['log-likelihood'] = result['log-likelihood'].tolist()
        ...
        # 动态生成保存路径
        # save_path = f'./json/k{k}.json'
        save_path = f'./json_quan/k{k}-{i}.json'
