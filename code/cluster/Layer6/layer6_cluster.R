...
norder <- c(seq(130,150,10),151:170)
bic<-c()
for (i in norder) {
  filename<-paste("D:/Soil3DNetworks/code/cluster/Layer6/json/k",i,"-",1,".json",sep = "")
...
for (i in 1:k) {
  ...
  for (ii in index){
    # points(fit6$Time,fit6$original_data[,ii],col="#7BABDD",pch=16)
    points(index6,data_order6[,ii],
           col=rgb(253,195,141,80,maxColorValue = 255),pch=16,cex=0.6)
    # lines(fit6$Time,fit6$power_fit[,ii],
    #        col=rgb(195,223,237,100,maxColorValue = 255),lwd=2)
  }
  
  if(length(index)==1){
    par <- power_equation_base2(fit6$Time, fit6$original_data[,index])
    lines(fit6$Time,power.equation(par,fit6$Time),col="#d64101",lwd=2)
  }else{
    par <- power_equation_base2(fit6$Time, rowMeans(fit6$original_data[,index]))
    lines(fit6$Time,power.equation(par,fit6$Time),col="#d64101",lwd=2)
  }
  box(lwd=1)
  if(i%in%((k-10+1):k)){
