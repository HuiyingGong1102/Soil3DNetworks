...
norder <- c(seq(400,430,10),431:450)
bic<-c()
for (i in norder) {
  filename<-paste("D:/Soil3DNetworks/code/cluster/Layer_all/json/k",i,"-",1,".json",sep = "")
  json_data <- fromJSON(filename)
  BIC <- json_data$BIC
  bic <- c(bic,BIC)
}

i=norder[which(bic==min(bic))]
filename<-paste("D:/Soil3DNetworks/code/cluster/Layer_all/json/k",i,"-",1,".json",sep = "")
json_data <- fromJSON(filename)
...
for (i in 1:k) {
  ...
  for (ii in index){
    ...
    points(indexa,data_order[,ii],
           col=rgb(213,225,227,30,maxColorValue = 255),pch=16,cex=0.6)
