...
norder <- c(seq(120,170,10),171:185)
bic<-c()
for (i in norder) {
  filename<-paste("D:/Soil3DNetworks/code/cluster/Layer3/json/k",i,"-",1,".json",sep = "")
  json_data <- fromJSON(filename)
  BIC <- json_data$BIC
  bic <- c(bic,BIC)
}

i=norder[which(bic==min(bic))]#178
filename<-paste("D:/Soil3DNetworks/code/cluster/Layer3/json/k",i,"-",1,".json",sep = "")
json_data <- fromJSON(filename)
...
for (i in 1:k) {
  ...
  for (ii in index){
    # points(fit3$Time,fit3$original_data[,ii],col="#7BABDD",pch=16)
    points(index3,data_order3[,ii],
           col=rgb(250,217,234,80,maxColorValue = 255),pch=16,cex=0.6)
    # lines(fit3$Time,fit3$power_fit[,ii],
    #        col=rgb(195,223,237,100,maxColorValue = 255),lwd=2)
  }
  
  if(length(index)==1){
    par <- power_equation_base2(fit3$Time, fit3$original_data[,index])
    lines(fit3$Time,power.equation(par,fit3$Time),col="#e22785",lwd=2)
  }else{
    par <- power_equation_base2(fit3$Time, rowMeans(fit3$original_data[,index]))
    lines(fit3$Time,power.equation(par,fit3$Time),col="#e22785",lwd=2)
  }
  box(lwd=1)
  if(i%in%((k-10+1):k)){
