...
data_df = pd.read_csv('log_data5.csv', index_col=0)
...
Time_df = pd.read_csv('log_time5.csv', index_col=0)
...
for k in list(range(161,162,1)):
    for i in list(range(21, 81, 1)):
        print(f"开始运行 k={k} 的分析...")
        result = fun_clu(data, k, Time, n1=15)
        result['par'] = result['par'].tolist()
        result['log-likelihood'] = result['log-likelihood'].tolist()
