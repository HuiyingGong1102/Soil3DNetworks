...
norder <- c(seq(100,190,10),seq(200,215,1))
bic<-c()
for (i in norder) {
  filename<-paste("D:/Soil3DNetworks/code/cluster/Layer1/json/k",i,"-",1,".json",sep = "")
  json_data <- fromJSON(filename)
  BIC <- json_data$BIC
  bic <- c(bic,BIC)
}

i=norder[which(bic==min(bic))]#210
filename<-paste("D:/Soil3DNetworks/code/cluster/Layer1/json/k",i,"-",1,".json",sep = "")
json_data <- fromJSON(filename)
...
library(jsonlite)
json_data <- fromJSON("D:/HuiyingGong1102/cluster/Layer1/json/k210-1.json")
k <- 210
module<-json_data$max_omega_logi
length(table(module))
table(module)

tiff("D:/Soil3DNetworks/code/cluster/layer1/module_curve.tiff",
     width=64,height=50,units = "cm",res=300)
par(mfrow=c(21,20),las=1)
par(oma = c(4, 4.5, 0.5, 0.5),mgp=c(1,0.5,0))

for (i in 1:k) {
  index<-which(module==(((1:k))-1)[i])
  
  y1 <- range(fit1$power_fit)
  lim2 <- y1[2]-y1[1]
  ...
  for (ii in index){
    # points(fit1$Time,fit1$original_data[,ii],col="#7BABDD",pch=16)
    points(index1,data_order1[,ii],
           col=rgb(195,223,237,50,maxColorValue = 255),pch=16,cex=0.6)
    # lines(fit1$Time,fit1$power_fit[,ii],
    #        col=rgb(195,223,237,100,maxColorValue = 255),lwd=2)
  }
  
  if(length(index)==1){
    par <- power_equation_base2(fit1$Time, fit1$original_data[,index])
    lines(fit1$Time,power.equation(par,fit1$Time),col="#084495",lwd=2)
  }else{
    par <- power_equation_base2(fit1$Time, rowMeans(fit1$original_data[,index]))
    lines(fit1$Time,power.equation(par,fit1$Time),col="#084495",lwd=2)
  }
  box(lwd=1)
  if(i%in%((k-10+1):k)){
