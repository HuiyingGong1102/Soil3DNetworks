...
norder <- c(seq(120,190,10),191:209)
bic<-c()
for (i in norder) {
  filename<-paste("D:/Soil3DNetworks/code/cluster/Layer2/json/k",i,"-",1,".json",sep = "")
  json_data <- fromJSON(filename)
  BIC <- json_data$BIC
  bic <- c(bic,BIC)
}

i=norder[which(bic==min(bic))]#197
filename<-paste("D:/Soil3DNetworks/code/cluster/Layer2/json/k",i,"-",1,".json",sep = "")
json_data <- fromJSON(filename)
...
for (i in 1:k) {
  ...
  for (ii in index){
    # points(fit2$Time,fit2$original_data[,ii],col="#7BABDD",pch=16)
    points(index2,data_order2[,ii],
           col=rgb(207,207,229,60,maxColorValue = 255),pch=16,cex=0.6)
    # lines(fit2$Time,fit2$power_fit[,ii],
    #        col=rgb(195,223,237,100,maxColorValue = 255),lwd=2)
  }
  
  if(length(index)==1){
    par <- power_equation_base2(fit2$Time, fit2$original_data[,index])
    lines(fit2$Time,power.equation(par,fit2$Time),col="#5f479a",lwd=2)
  }else{
    par <- power_equation_base2(fit2$Time, rowMeans(fit2$original_data[,index]))
    lines(fit2$Time,power.equation(par,fit2$Time),col="#5f479a",lwd=2)
  }
  box(lwd=1)
  if(i%in%((k-10+1):k)){
